...
PowerDiag reuses BatteryGuard's existing approximately-10-second `Board::power_status()` safety sample. BatteryGuard returns the same sampled `PowerStatus` in `UpdateResult`; no second PowerDiag PMU poll is introduced. Valid samples retain battery start/current/minimum voltage, start/current percentage, signed voltage delta, latest charge state, and the latest supported AXP192 signed battery-current reading. While a PowerDiag session is Running, the same valid/current-supported samples also accumulate a signed arithmetic current mean and sample count. START/NEW TEST resets that sum/count, and a cached pre-start current is excluded from the session mean. Battery current remains diagnostic only: 0 mA is a legitimate neutral reading, charging/discharging sign is preserved, no FULL/error heuristic is inferred from current, and BatteryGuard does not consume current for advisory or shutdown decisions. Invalid battery samples do not affect battery baseline/minimum or current-average calculations.
...
## D-039 — Charging Mode is a VBUS-owned local-use lock with incoming priority

**Status:** Accepted for hardware validation

Charging Mode v0.1 is owned at the system-composition level in `app_main`. Board exposes only raw M5Unified/AXP192 VBUS voltage and theme-aware drawing primitives; product thresholds and session state remain above Board. Physical cable presence is sampled approximately every 1000 ms and uses `VBUS >= 4000 mV`. `isCharging()` alone is explicitly not cable presence because the PMIC may stop active charging while USB power remains connected.

While VBUS is present, Charging Lock consumes ordinary local M5, BOCZNY, and short POWER interactions before normal `DisplayLifecycle`/application input. Initial cable detection and each ordinary short interaction show the current charging presentation for 5000 ms at the selected normal Active brightness, after which the LCD goes directly to DisplayOff. Launcher/apps/settings/tools therefore remain locally inaccessible while locked. Incoming PresetMessage/PresetResponse and RING/SYGNAL are the deliberate exception: they use the normal Communicator wake, notification, and interaction UI, then return directly to Charging Lock instead of exposing Communicator Main/Options.

The charging presentation is deliberately minimal: a large theme-aware battery plus `LADOWANIE`, animated through four fill steps only while the LCD is visible. Confirmed Full uses a large filled battery plus `OK`. `OK` does not mean certified safe-to-unplug operation, cell-temperature verification, or any broader safety guarantee; this implementation does not measure cell temperature directly.

Full is not derived from displayed percentage, one high-voltage sample, one non-Charging sample, or battery current. The current cable session must first observe a valid BatteryGuard sample with VBUS present and `ChargeState::Charging`. After that, Full requires three consecutive valid BatteryGuard samples with VBUS >= 4000 mV, battery voltage >= 4100 mV, and `ChargeState::Discharging`. Any Charging sample, invalid/low battery voltage, unknown charge state, or absent VBUS resets in-progress confirmation. The existing approximately-10-second BatteryGuard cadence therefore supplies the evidence without duplicate battery-voltage/current polling.

Full confirmation still latches immediately for the cable session, but presentation authority belongs to Charging Lock. If an incoming communication or Owner Override owns the foreground when Full is confirmed, the completion presentation is marked pending without tone or screen theft. When Charging Lock next regains control, Board emits one neutral approximately-3200 Hz / 90 ms tone and shows Full/OK for a fresh 5-second window exactly once. Later button checks show Full/OK without repeating the tone. VBUS removal clears both the latch and any still-deferred presentation event.

Countdown timing, pending expiration, PowerDiag battery-current semantics, BatteryGuard LOW/VERY LOW/critical policy, Communicator messaging lifecycle, Presence, retry/ACK/TxResult, RadioLab ownership, and the D-037 BG 3000/500 / delivery-boost 1000/500 RX schedules remain unchanged. Messaging continues using its existing background service behavior during Charging Mode. Because the incoming queue is only a bounded transport handoff, Charging Lock services it continuously through Communicator-owned shared drain/validation/latest-wins logic. Normal user messages and RING retain their normal presentation semantics; while an existing RING already owns the screen, the same drain still collapses newer user traffic underneath it so queue occupancy cannot remain stuck at four.
