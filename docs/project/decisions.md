...
## D-039 — Charging Mode is VBUS-owned system presentation with conservative Full confirmation
...
While VBUS is present, Charging Mode consumes short M5, BOCZNY, and POWER interactions before normal `DisplayLifecycle`/application input. Initial cable detection and each accepted short interaction show the current charging presentation for 5000 ms at the selected normal Active brightness, after which the LCD goes directly to DisplayOff. Charging Mode therefore replaces Clock Glance and ordinary UI presentation while connected without changing the global 15 s / 45 s display lifecycle.
...
On the exact Full transition, Board emits one neutral approximately-3200 Hz / 90 ms tone, the LCD wakes, and the Full/OK view receives a fresh 5-second visibility window. Full then remains latched for the cable session; later button checks show Full/OK without repeating the tone, even if maintenance charging behavior changes. VBUS removal clears the complete charging/full session and restores the pre-existing runtime UI without reboot or application-state reset.

Countdown timing, pending expiration, PowerDiag battery-current semantics, BatteryGuard LOW/VERY LOW/critical policy, Communicator messaging lifecycle, Presence, retry/ACK/TxResult, RadioLab ownership, and the D-037 BG 3000/500 / delivery-boost 1000/500 RX schedules remain unchanged. Messaging continues using its existing background service behavior during Charging Mode. Because the messaging incoming queue is only a bounded transport handoff, Charging Mode must not leave it undrained: while Communicator messaging is enabled, `app_main` invokes a Communicator-owned silent drain after normal transport service. The drain shares the normal validation/latest-wins selection logic, silently applies only the newest valid PresetMessage/PresetResponse to Communicator state, and consumes/discards RING/SYGNAL without tone, render, wake, or deferred presentation.

**Rationale:** VBUS is the stable authority for whether the cable-powered presentation should remain active, while conservative BatteryGuard-sourced completion evidence avoids a false `OK` from voltage-derived 100% or transient PMIC state. Keeping the feature in existing system composition preserves application/radio ownership and avoids a new power-management framework.
