...
- Communicator v0.1 is now a latest-wins non-blocking pager: sends never wait for human response, responses are self-contained PresetId + ResponseId messages, the newest received user message replaces older unanswered content, every received preset is skippable, CZEKAC? remains a normal follow-up preset, and delivery ACK remains device-delivery state only. Delivery feedback is a small non-modal WYSYLAM/DOSTARCZONO/NIE DOSTARCZONO banner that can follow ordinary Launcher navigation without waking DisplayOff or obscuring higher-priority UI. SYGNAL, volatile service lifecycle, and STANDARD/LR ownership remain unchanged; RX profile choice is now messaging-delivery-owned rather than Communicator-UI-owned.
- Settings provides volatile `Lagodny / Klasyczny / Pager` signal-sound selection, `Niska / Srednia / Wysoka` display brightness (72/18, 96/24 default, 128/32), a scrollable volatile `Nikos / Bursztyn / Matrix / Lava / Noir` visual-theme selection, and immediate `PRAWA / LEWA` display orientation. Brightness applies immediately through Board and remains boot-scoped; orientation is Board-wide without changing logical button roles. Full reboot restores Lagodny, Srednia, Nikos, and PRAWA defaults.
