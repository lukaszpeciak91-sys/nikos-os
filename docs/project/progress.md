...
- Communicator protocol v2 now sends only type-specific semantic-ID payloads: Presence 2 B, Ring/ACK 6 B, PresetMessage 7 B, and PresetResponse 11 B; text remains local, 32-bit logical IDs remain, and v1 compatibility is intentionally removed.
...
- 2026-09-21 — Replaced the fixed 20-byte Communicator v1 frame with v2 type-specific application payloads (2/6/6/7/11 B), preserved 32-bit logical IDs and stable semantic catalogue IDs, and moved all send call-sites to the actual encoded length; both sticks must run the same v2 firmware.
