...
- Communicator v0.1 is now a latest-wins non-blocking pager: sends never wait for human response, responses are self-contained PresetId + ResponseId messages, the newest received user message replaces older unanswered content, every received preset is skippable, CZEKAC? remains a normal follow-up preset, and delivery ACK remains device-delivery state only. SYGNAL, foreground/background profiles, volatile service lifecycle, and STANDARD/LR ownership remain unchanged.
