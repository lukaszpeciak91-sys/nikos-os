...
- Charging Mode v0.1 is a VBUS-owned system presentation: physical VBUS is sampled about once per second, connected mode hides ordinary UI, M5/BOCZNY/short POWER show a large theme-aware charging/full view for 5 s, and the LCD then goes directly Off. Full requires prior observed Charging plus three consecutive BatteryGuard samples with VBUS >=4000 mV, battery >=4100 mV, and PMIC Discharging; it beeps once, latches until unplug, and does not use percentage/current or claim cell-temperature verification. While charging, enabled Communicator traffic is silently drained through the same latest-wins validation path so normal user messages remain current without UI/audio/wake and RING/SYGNAL is consumed as non-deferred transient traffic. BatteryGuard safety and Communicator radio policy are unchanged.
...
- 2026-09-25 — Added Charging Mode v0.1: Board exposes raw VBUS and PowerStatus carries it on the existing BatteryGuard sample; app_main owns 1 s VBUS presence, 5 s charging/full presentation and input interception, conservative three-sample Full confirmation with a one-shot tone/latch, silent latest-wins Communicator queue draining with RING suppression, visible fatal NVS diagnostics even after charging boot sleep, and normal-runtime restoration on unplug without changing BatteryGuard or radio/messaging policy.
