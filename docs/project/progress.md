...
- Narzedzia contains RadioLab, PowerDiag, and Powrot. PowerDiag provides a RAM-only background diagnostic session with two compact pages; leaving its UI does not stop the session. RadioLab and PowerDiag return to Narzedzia after normal exit. Rozrywka now contains Snake and Powrót; Snake is a dedicated fixed-memory foreground app with relative two-button steering, wrap-around edges, RAM-only runtime record, tunable monotonic movement speed, and a 10-minute per-entry play limit. Zegar now shows RTC-backed HH:MM (or --:--), USTAW CZAS, MINUTNIK, STOPER, and POWROT. Ustawienia contains Dzwiek, Motyw, Orientacja, and Powrot.
