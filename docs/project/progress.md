...
- Communicator v0.1 now provides one-peer availability, a five-message Polish preset catalogue, one-preset/one-response-at-a-time physical-LCD presentation, shallow response-ending semantics without mandatory HumanOk, the meaningful `ZACZEKAĆ?` follow-up, collision restoration driven by technical delivery, short audible notification, foreground/background messaging profile switching, a separate orange `SYGNAŁ` attention action, explicit volatile session enable/disable lifecycle, and a minimal volatile `STANDARD/LR` radio-mode option owned by messaging.
